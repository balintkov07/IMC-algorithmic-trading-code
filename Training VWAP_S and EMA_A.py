...
class Trader:
  ...
  def run(self, state: TradingState):
    ...
    for product in state.order_depths:
      ...
      print(product + ": Buy orders (OrderDepths): " +
            str(order_depth.buy_orders))
      print(product + ": Sell orders (OrderDepths): " +
            str(order_depth.sell_orders))
      ...
      if product == "STARFRUIT":
        ...
        acceptable_price = (self.offer_VWAP + self.bid_VWAP
                            ) / 2  # Participant should calculate this value
      ...
      if product == "AMETHYSTS":
        ...
        ema = self.calculate_ema(self.prices_storer, 20, 0)

        if len(order_depth.sell_orders) != 0:
          for price, volume in order_depth.sell_orders.items():
            if price <= ema[-1] - deviation:
              max_buy = self.position_limit - self.position
              orders.append(Order(product, price, min(-volume, max_buy)))
              self.position += min(-volume, max_buy)
